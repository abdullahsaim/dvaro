...
/*
 * DVARO backup destination:
 *   - 'backup'       → Contabo Object Storage — used ONLY when its credentials
 *                      (CONTABO_BACKUP_KEY) are configured.
 *   - 'backup-local' → on-VPS fallback (storage/backups) when no Contabo creds.
 * Both disks are defined in config/filesystems.php. The app 's3' bucket is
 * NEVER a backup target (dedicated backup bucket, per CLAUDE.md).
 */
$backupDisk = env('CONTABO_BACKUP_KEY') ? 'backup' : 'backup-local';
...
return [
    ...
    'backup' => [
        ...
        'destination' => [
             ...
             * Contabo Object Storage when configured, else the local fallback.
             */
            'disks' => [
                $backupDisk,
            ],
