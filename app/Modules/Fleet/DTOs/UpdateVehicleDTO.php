...
/**
 * Carries validated vehicle-edit data into UpdateVehicleAction.
 *
 * Deliberately has NO status field: a vehicle's status is mutated ONLY through
 * ChangeVehicleStatusAction (via the separate changeStatus endpoint), never on
 * a general edit. daily_rate is in CENTS — matches vehicles.daily_rate.
 */
class UpdateVehicleDTO extends BaseDTO
{
    ...
    /**
     * Attributes for $vehicle->update(). NOTE: no 'status' key — status changes
     * go exclusively through ChangeVehicleStatusAction.
     *
