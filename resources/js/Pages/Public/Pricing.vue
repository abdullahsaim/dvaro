    ...
    <PublicLayout>
        ...
        <section class="mx-auto w-full max-w-7xl px-4 py-20 sm:px-6 lg:px-8">
            <div class="mx-auto max-w-2xl text-center">
                <h1 class="text-4xl font-bold tracking-tight text-ink-900 dark:text-ink-50">{{ t('public.pricing.title') }}</h1>
                <p class="mt-3 text-ink-600 dark:text-ink-300">{{ t('public.pricing.subtitle') }}</p>
            </div>

            <!-- Monthly / Annual toggle -->
            <div class="mt-8 flex justify-center">
                <div class="inline-flex rounded-control border border-ink-200 p-1 dark:border-ink-800">
                    <button
                        type="button"
                        class="rounded-md px-4 py-1.5 text-sm font-medium transition"
                        :class="cycle === 'monthly' ? 'bg-ink-950 text-white dark:bg-ink-50 dark:text-ink-950' : 'text-ink-600 dark:text-ink-300'"
                        @click="cycle = 'monthly'"
                    >
                    ...
                    <button
                        type="button"
                        class="rounded-md px-4 py-1.5 text-sm font-medium transition"
                        :class="cycle === 'annual' ? 'bg-ink-950 text-white dark:bg-ink-50 dark:text-ink-950' : 'text-ink-600 dark:text-ink-300'"
                        @click="cycle = 'annual'"
                    >
